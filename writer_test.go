		...
		for i, file := range decodeTestFiles {
			...
			t.Run(fmt.Sprintf("%s/%s", file.folder, file.name), func(t *testing.T) {
				...
				if refp != fp {
					...
					err = fitUtterDump(reFile, fmt.Sprintf("%s-%s.current", file.folder, file.name), false)
					if err != nil {
					...
					fpath = fpath + ".in"
					err = fitUtterDump(inFile, fmt.Sprintf("%s-%s.current.in", file.folder, file.name), false)
					if err != nil {
						t.Fatalf("error writing output: %v", err)
					}
